...
//	@Length(max = 20, min = 3 ,message = "Book Title lenght should be in between 3 to 20 characters.")
  ...
  @Min(value = 1, message = "Stock should be greater than 1")
	...
	public BookDTO(
//			@NotBlank @Length(max = 20, min = 3, message = "Book Title lenght should be in between 3 to 20 characters.") 
			String title,
			List<String> languageNames, String bookImage, LocalDate publicationDate,
			@NotBlank @Length(max = 20, min = 3, message = "Author Name should be in between 3 to 20 characters.") String author,
			...
			@Min(value = 1, message = "Stock should be greater than 1") int stock, Genre genre,String description) {
		super();
		this.title = title;
